...
class ForumCrawler:
    ...
    def run_login(self, login_url, custom_payload=None):
        if not self.username or not self.password:
            return
        payload = custom_payload or {"vb_login_username": self.username, "vb_login_password": self.password, "securitytoken": "guest", "do": "login"}
        try:
    ...
    def scrape_thread_pages(self, initial_url, thread_id, thread_title):
        ...
        while current_url:
            ...
            try:
                ...
                posts = []
                for s in post_selectors:
                    posts.extend(soup.select(s))
